...
- `sketch.js` (0) - custom image classification
  ...
  - 사람의 신체 부위가 지나치게 개입된 상태로 이미지 추가를 하지 말 것
  - 이미지 추가 시 개입된 사람의 신체에 비하여 상대적으로 작은 물체는 이미지에서 점유하는 픽셀이 작기때문에 판별하기 어려움
...
- [ ] 실습: 행복한(happy) 표정과 슬픈(sad) 표정을 학습시키고 인공지능이 나의 감정 상태를 구별하도록 해보자.
