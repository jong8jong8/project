...
function setup() {
    ...
    mobilenet = ml5.imageClassifier('MobileNet', modelReady_nB);
    ...
    speech.setLang('en-US');
    button = createButton('click to calssify');
    button.position(width+100, height);
    button.mousePressed(modelReady);    
}

function modelReady_nB() {
    console.log('Model is ready!');
    mobilenet.classify(img, gotResults_nB);    
}

function gotResults_nB(error, results) { 
    if (error) {
        console.log(error);
    } else {
        console.log(results);
